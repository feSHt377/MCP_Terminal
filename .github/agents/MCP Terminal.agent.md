...
1. Call `launch_gui` only when the mcpterminal GUI is not already running.
2. Call `list_sessions` to inspect existing SSH sessions. Treat this as read-only discovery; it must not create a session.
